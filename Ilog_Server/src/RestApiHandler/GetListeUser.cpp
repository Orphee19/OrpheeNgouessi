...
HttpResponse GetListeUser::process(HttpRequest* req)
{
    auto psql_= getPSQL();
    ...
    {        
        ...
        std::string query = "SELECT * FROM utilisateur WHERE";
        uint add = 0;
        for (auto i = (int)UserProfil::USER; i < (int)UserProfil::DEV; i++)
        {
            if (i < prof)
            {
                if (add > 0)
                ...
                query += " profil = '" + UserProfilStringifier.toString(UserProfil(i)) + "'";
                add++;
            }
        }
        ...
        for (const auto item : res)
        {
            ...
            json["username"] = (res[0][0].is_null()) ? "" : res[0][0].as<std::string>();
            json["sexe"] = (res[0][1].is_null()) ? 0 : res[0][1].as<int>();
            json["nom"] = (res[0][2].is_null()) ? "" : res[0][2].as<std::string>(); 
            json["prenom"] = (res[0][3].is_null()) ? "" : res[0][3].as<std::string>();
            json["email"] = (res[0][4].is_null()) ? "" : res[0][4].as<std::string>();
            json["status"] = (res[0][6].is_null()) ? 0 : res[0][6].as<int>();
            json["profil"] = (res[0][7].is_null()) ? "" : res[0][7].as<std::string>();
